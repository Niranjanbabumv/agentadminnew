...
var userData = "";
	dbForApplicantData.list({ include_docs: true },function(err, body) {
	  if (!err) {
...
console.log("Got a POST request for StudentDetails.html page");
	...
	dbForAdminRequestTable.list({ include_docs: true },function(err, body) {
	  if (err) {
		...
		dbForAdminRequestTable.insert({ _id: id, _rev: revId, status: newstatus, digitalId: digitalid, requestDate: requestdate, applicantName: applicantname }, function(err, body) {
			  if (!err){
					var response = {
						status  : 200,
